...
const PASS = false, FAIL = true // eslint-disable-line one-var
...
describe('Stock General Validators', () => {
  ...
  describe('required validator', () => {
    ...
    it('handles array', () => {
      ...
      return Promise.all(arrs.map(async arr => runValidator(schema, arr, !arr)))
    })
...
describe('Stock String Validators', () => {
  ...
  describe('length validator', () => {
    ...
    it('handles arrays', async () => {
      ...
      return runValidator(schema, arr, arr.map(str => str.length <= 5 ? false : 'Bad.'))
    })
